...
REM 32-bit libs (skip if not available - only needed for x86 builds)
COPY /V /Y "%MPCHC_MINGW64%\lib\gcc\i686-w64-mingw32\%gccver%\libgcc.a" %LIBDIR%\ 2>NUL

REM 64-bit libs
COPY /V /Y "%MPCHC_MINGW64%\lib\gcc\x86_64-w64-mingw32\%gccver%\libgcc.a" %LIBDIR%64\

REM Try standard cross-compile path first, then direct lib path (MSYS2 layout)
COPY /V /Y "%MPCHC_MINGW64%\i686-w64-mingw32\lib\libmingwex.a" %LIBDIR%\ 2>NUL
IF NOT EXIST "%LIBDIR%\libmingwex.a" COPY /V /Y "%MPCHC_MINGW64%\lib\libmingwex.a" %LIBDIR%\ 2>NUL
IF EXIST "%LIBDIR%\libmingwex.a" (
...
COPY /V /Y "%MPCHC_MINGW64%\x86_64-w64-mingw32\lib\libmingwex.a" %LIBDIR%64\ 2>NUL
IF NOT EXIST "%LIBDIR%64\libmingwex.a" COPY /V /Y "%MPCHC_MINGW64%\lib\libmingwex.a" %LIBDIR%64\
